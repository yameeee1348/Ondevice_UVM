...
//    end
    ...
    always_comb begin 
        case ({cpol, cpha})
            2'b00: begin // Mode 0
                sample_edge = sclk_rise; // Leading (Rise)
                drive_edge  = sclk_fall; // Trailing (Fall)
            end
            2'b01: begin // Mode 1
                sample_edge = sclk_fall; // Trailing (Fall)
                drive_edge  = sclk_rise; // Leading (Rise)
            end
            2'b10: begin // Mode 2
                sample_edge = sclk_fall; // Leading (Fall)  <-- 수정됨
                drive_edge  = sclk_rise; // Trailing (Rise) <-- 수정됨
            end
            2'b11: begin // Mode 3
                sample_edge = sclk_rise; // Trailing (Rise) <-- 수정됨
                drive_edge  = sclk_fall; // Leading (Fall)  <-- 수정됨
            end 
            default: begin
                sample_edge = 1'b0;
                drive_edge  = 1'b0;
